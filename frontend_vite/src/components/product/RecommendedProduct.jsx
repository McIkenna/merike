...
const RecommendedProduct = ({ recommendedProducts, handleSelect }) => {
  ...
  return (
    <StyledCard onClick={() => handleSelect?.('recommended')}>
      <HeaderBox>
        <GradientText>
          Recommended for you
        </GradientText>
      ...
      <Box sx={{ position: 'relative', zIndex: 1, height: 'calc(100% - 80px)' }}>
        ...
        >
          {recommendedProducts.slice(0, 8).map((item, index) => (
            <StyledImageListItem
              key={index}
            ...
            >
              <img
                srcSet={`${item?.images[0].url}?w=400&h=400&fit=crop&auto=format&dpr=2 2x`}
                src={`${item?.images[0].url}?w=400&h=400&fit=crop&auto=format`}
                alt={item?.name}
                loading="lazy"
